...
aviso_img = pygame.image.load('aviso5.png').convert_alpha()
vida_img = pygame.image.load('vida.png').convert_alpha()
portal_img = pygame.image.load('portal_mar.png').convert_alpha()
mascara_img = pygame.image.load('mascara.png').convert_alpha()
...
class Personagem(pygame.sprite.Sprite):
    ...
    def update(self, movimento, plataformas, chaos, inimigos, portals, placas):
...
def desenhar_dica(tela):
    tela.blit(mascara_img, (10 + 4 * 40, 5))
...
def jogo():
    ...
    posicoes_masks = [
        (1900, SCREEN_HEIGHT - 125),
    ]
    for x, y in posicoes_masks:
        masks.add(Mask(x, y))
    ...
    while rodando:
        ...
        personagem.update(movimento, plataformas, chaos, inimigos, portals, placas)
        ...
        for mask in masks:
            mask.rect.x -= movimento_horizontal

        # Detectar interação com mask
        if pygame.sprite.collide_rect(personagem, mask):
            global mostrar_mask
            mostrar_mask = True
            masks.remove(mask)
